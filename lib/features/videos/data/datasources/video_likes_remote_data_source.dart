import 'package:uwifiapp/features/videos/data/models/like_response_model.dart';

/// Interfaz para el datasource remoto de likes de videos
abstract class VideoLikesRemoteDataSource {
  /// Registra un like de un usuario para un video específico
  ///
  /// [customerId] ID del cliente que da like
  /// [videoId] ID del video (media_file_id) al que se da like
  /// 
  /// Retorna un [LikeResponseModel] con el resultado de la operación
  Future<LikeResponseModel> likeVideo(int customerId, String videoId);
  
  /// Elimina un like de un usuario para un video específico
  ///
  /// [customerId] ID del cliente que elimina el like
  /// [videoId] ID del video (media_file_id) del que se elimina el like
  /// 
  /// Retorna un [LikeResponseModel] con el resultado de la operación
  Future<LikeResponseModel> unlikeVideo(int customerId, String videoId);
  
  /// Verifica si un usuario ha dado like a un video específico
  ///
  /// [customerId] ID del cliente a verificar
  /// [videoId] ID del video (media_file_id) a verificar
  /// 
  /// Retorna true si el usuario ha dado like al video, false en caso contrario
  Future<bool> hasUserLikedVideo(int customerId, String videoId);
}
